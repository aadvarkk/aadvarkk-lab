        ...
        try:
            ...
            if True:
                ...
                if content_length:
                    ...
                    handler, transforms, args, kwargs = self.request_callback.prepare_call(self._request)
                    if isinstance(handler, tornado.web.ChunkedRequestHandler):
                        self.stream.read_bytes(content_length, handler.on_chunk_finished, handler.on_chunk_rcvd)
                    else:
                        print type(handler)
                        assert False
                        self.stream.read_bytes(content_length, self._on_request_body)
                    self.request_callback.exec_handler(handler, transforms, args, kwargs)
                    return
            else:
                # ORIG
                content_length = headers.get("Content-Length")
